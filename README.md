...
1. **장소 검색 → 선택** - 키워드로 장소 검색 후 기준점 선택 (식당, 역, 구청 등 모든 장소)
2. **주변 음식점 수집** - 5방향 분할 검색으로 반경 500m 내 음식점 수집 (API 45개 제한 우회)
3. **가짜 리뷰 탐지** - 9가지 규칙으로 리뷰 조작 여부 분석 (0~100% 의심도)
4. **종합 스코어링** - 즐겨찾기 매칭(+100), 평점(최대 30점), 리뷰 수(최대 20점), 페널티
...
| 5 | 세글자 닉네임 연속 | 10 | 전체 리뷰 타임라인에서 세글자 한글 닉네임 5점이 연속 3건 이상 |
...
규칙 5번은 전체 리뷰 타임라인에서 **연속성(temporal contiguity)**을 검사합니다. 한국인 이름이 자연적으로 세글자가 많으므로, 빈도가 아닌 연속 출현이 핵심 신호입니다.
...
# 즐겨찾기 관리
uv run matjip fav list                    # 전체 목록
uv run matjip fav list -s "강남"           # 검색 (한영 주소 매핑 지원)
uv run matjip fav list -s "강남" -n 100    # 결과 수 제한
...
2. 우측 상단 **맛집 어시스턴트** 패널에서 검색어 입력 (식당뿐 아니라 지하철역, 구청 등 아무 장소 검색 가능)
3. 검색 결과에서 **기준 장소를 선택** (2단계 검색 UX)
4. 선택한 장소 반경 500m 내:
   - 음식점 리뷰 분석 + 의심도 기반 랭킹
   - 즐겨찾기에 등록된 장소도 함께 표시 (⭐ 마커)
   - 지도에 번호 마커 표시 (의심도별 색상: 초록/노랑/빨강)
   - 마커 hover 시 최상위 표시 + 클릭 시 상세 페이지 이동
5. [place.map.kakao.com](https://place.map.kakao.com) 개별 식당 페이지에서 자동 분석 배지 표시 (SPA 네비게이션 자동 감지)
...
| API | 용도 | 인증 |
|-----|------|------|
| `dapi.kakao.com/v2/local/search/keyword.json` | 장소 키워드 검색 | REST API 키 |
| `dapi.kakao.com/v2/local/search/category.json` | 주변 카테고리 검색 (FD6=음식점, 5방향 분할) | REST API 키 |
...
>
> **참고**: 카테고리 검색 API는 `pageable_count`가 45개로 하드 리밋되어 있습니다. 이를 우회하기 위해 5방향 분할 검색(중심+동서남북)을 사용합니다.
...
| `favorites.json` | 즐겨찾기 목록 (`group_name` 포함) |
| `folder_names.json` | 카카오맵 즐겨찾기 폴더 이름 (그룹명 매핑) |
